...
public class PlayGame extends JFrame implements KeyListener {
		...
		try {
			...
			for(int i = 0; i < board_width; i++)
				for(int j = 0; j < board_length; j++) {
					//gameDisplay[i][j] = new GameButton("",emptyTile);
	...
	/* ===================================== */ 		
		...
		redrawTile(prevHead.getRow(),prevHead.getCol(),emptyTile);
	...
	/* ======================== */
		...
		while (true) {
			...
			if ((row != rowHead) && (col != colHead) && board.getType(row,col).equals("empty")
	...
	private void printDebugInfo() {
		...
		for (int i = 0; i < board_width; ++i) {
			...
			for (int j = 0; j < board_length; ++j) {
				System.out.print(board.getType(i,j));
